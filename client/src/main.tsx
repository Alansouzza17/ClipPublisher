...
function App() {
  ...
  const summary=useMemo(()=>({tk:videos.filter(v=>v.publishToTikTok).length,yt:videos.filter(v=>v.publishToYouTube).length}),[videos]);
  ...
  <footer><div><span>{videos.length} vídeos serão publicados · TikTok: {summary.tk} · YouTube Shorts: {summary.yt}</span><nav className="legal-links"><a href="/terms/">Termos de Serviço</a><a href="/privacy/">Política de Privacidade</a></nav></div><button className="publish" onClick={()=>setConfirm(true)}>🚀 PUBLICAR TODOS</button></footer></>}{confirm&&<div className="modal"><div><h2>Publicar agora?</h2><p>{videos.length} vídeos serão enviados às plataformas selecionadas.</p><button onClick={()=>setConfirm(false)}>Cancelar</button><button className="publish" onClick={publishAll}>Publicar</button></div></div>}</main> }
