...
const dataDir=process.env.DATA_DIR?path.resolve(process.env.DATA_DIR):path.resolve('storage');
export const config={port:Number(process.env.PORT||3001),frontendUrl:process.env.FRONTEND_URL||'http://localhost:5173',maxUploadBytes:Number(process.env.MAX_UPLOAD_BYTES||1073741824),tempDir:path.join(dataDir,'uploads'),tokenFile:path.join(dataDir,'tokens.enc'),tokenKey:process.env.TOKEN_ENCRYPTION_KEY||'',tiktok:{key:process.env.TIKTOK_CLIENT_KEY||'',secret:process.env.TIKTOK_CLIENT_SECRET||'',redirect:process.env.TIKTOK_REDIRECT_URI||''},google:{id:process.env.GOOGLE_CLIENT_ID||process.env.YOUTUBE_CLIENT_ID||'',secret:process.env.GOOGLE_CLIENT_SECRET||process.env.YOUTUBE_CLIENT_SECRET||'',redirect:process.env.GOOGLE_REDIRECT_URI||''}};
