import crypto from 'node:crypto'; import fs from 'node:fs/promises';import path from 'node:path'; import {config} from './config.js'; import type {Token} from './types.js';
...
export async function saveTokens(platform: string, tokens: Token) {
  if (!config.tokenKey) {
    throw new Error('TOKEN_ENCRYPTION_KEY não configurada.');
  }

  await fs.mkdir(path.dirname(config.tokenFile), {
    recursive: true,
  });

  let all: Record<string, Token> = {};
